...
export interface AppState {
  ...
  updateSingleTaskState: (taskId: string) => Promise<void>;
}
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  updateSingleTaskState: async (taskId: string) => {
    try {
      // Fetch the specific task with all nested elements
      ...
      if (!taskData) {
        // If the task does not exist (deleted), remove it from both lists
        set((state) => ({
      ...
      // 1. Process task for global tasks list
      const processTaskForGlobal = (tRaw: any) => {
        const uniqueSubs = (tRaw.subtasks || []).filter((sub: any, index: number, self: any[]) =>
          self.findIndex((s: any) => s.id === sub.id) === index
        );
        const task = { ...tRaw, subtasks: uniqueSubs };
      ...
      // 2. Process task for Daily tasks (filtering logs with state's startDate / endDate)
      const processTaskForDaily = (tRaw: any, startDate: string, endDate?: string) => {
        const uniqueSubs = (tRaw.subtasks || []).filter((sub: any, index: number, self: any[]) =>
          self.findIndex((s: any) => s.id === sub.id) === index
        );
        const task = { ...tRaw, subtasks: uniqueSubs };
        ...
        // Filter logs on target date range locally
        let taskLogs = task.task_logs || [];
        let subtaskLogs = task.subtask_logs || [];

        if (startDate) {
          if (endDate) {
            taskLogs = taskLogs.filter((log: any) => log.todo_date >= startDate && log.todo_date <= endDate);
            subtaskLogs = subtaskLogs.filter((log: any) => log.todo_date >= startDate && log.todo_date <= endDate);
          } else {
            taskLogs = taskLogs.filter((log: any) => log.todo_date === startDate);
            subtaskLogs = subtaskLogs.filter((log: any) => log.todo_date === startDate);
          }
        }
      ...
      const state = get();
      const processedGlobal = processTaskForGlobal(taskData);
      const processedDaily = processTaskForDaily(taskData, state.startDate, state.endDate || undefined);

      // Merge into local states cleanly
      set((prev) => {
        let nextTasks = [...prev.tasks];
        const globalIdx = nextTasks.findIndex((t) => t.id === taskId);
        if (globalIdx !== -1) {
          nextTasks[globalIdx] = processedGlobal;
        } else {
          nextTasks.unshift(processedGlobal);
        }

        let nextDailyTasks = [...prev.dailyTasks];
        const dailyIdx = nextDailyTasks.findIndex((t) => t.id === taskId);
        if (dailyIdx !== -1) {
          nextDailyTasks[dailyIdx] = processedDaily;
        } else {
          nextDailyTasks.unshift(processedDaily);
        }
      ...
      console.error('Error handling incremental task update:', err);
    }
  }
